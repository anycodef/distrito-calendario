...
export default function LiderSidebar() {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 w-64 bg-slate-900 shadow-xl flex flex-col text-white z-50">
      <div className="flex h-16 shrink-0 items-center px-6 border-b border-slate-800 bg-slate-950">
        <h1 className="text-xl font-bold tracking-tight">Panel de Liderazgo</h1>
      </div>
      <nav className="flex-1 space-y-2 p-4 overflow-y-auto">
        {navItems.map((item) => {
          const isActive = pathname === item.href || (pathname.startsWith(item.href) && item.href !== "/dashboard/lider");
          return (
            <Link
              key={item.name}
              href={item.href}
